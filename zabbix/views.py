...
def show_graph(username):
	...
	for each in group_info:
		prompt += "%-10s : 网卡 <a href='http://zabbix.tansuotv.cn/wx_api/network/%s/1'>eth1</a>\t<a href='http://zabbix.tansuotv.cn/wx_api/network/%s/0'>eth0</a> " \
		          "<a href='http://zabbix.tansuotv.cn/wx_api/load/%s'>查看系统负载</a>\n" % (each,each,each,each)
	return prompt
